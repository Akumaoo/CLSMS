...
$table_MS="";
	...
	$table_MS=$('#table_MS').DataTable({			
	"processing":true,
	"serverSide":true,
	"ordering":true,
	"searching":true,
	"ajax":"SSP/recycle/serverside_manage_serials.php"
		});
	}
...
$('#table_MS').on('draw.dt', function() {
		...
		else
		{
			...
			if(!$checkele)
			{
				$('thead>tr').append('<th id="th_add" class="radio-label-center" tabindex="0" aria-controls="table_MS" rowspan="1" colspan="1" style="width: 63px; font-size:10px">Check All <br><input type="checkbox" id="SA_ser" value="SA"> </th>');
	...
	$('#RS').click(function(){
 		...
 		if(CheckList())
 		{
	 		...
	 		$.ajax({
	 			url:"php_codes/recycle/modify_serials.php",
	 			method:"POST",
