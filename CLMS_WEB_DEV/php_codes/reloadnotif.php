     ...
     $initial_notif_launch="Select [Serial].[SerialName],[Notification].[NotificationType],[Notification].[NotificationSeen] from Serial Inner Join [Notification] ON [Serial].[SerialID]=[Notification].[SerialID] Where [Notification].[NotificationSeen]=?";
      ...
      if(sqlsrv_has_rows($query))
      {
        while($row=sqlsrv_fetch_array($query,SQLSRV_FETCH_ASSOC))
        {
          ...
          {
            $delivery_sqltxt="Select [Serial].[SerialName],[Notification].[NotificationType],[Notification].[NotificationSeen],[Distributor].[DistributorName],[Delivery].[ExpectedReceiveDate],[Notification].[Date_Receive_RedFlag] from Serial Inner Join [Notification] ON [Serial].[SerialID]=[Notification].[SerialID] Right JOIN Subscription ON [Notification].[SerialID]=[Subscription].[SerialID] INNER JOIN Delivery ON [Subscription].[DistributorID]=[Delivery].[DistributorID] LEFT JOIN [Distributor] ON [Delivery].[DistributorID]=[Distributor].[DistributorID] Where [Notification].[NotificationSeen]=? AND [Notification].[NotificationType]=? AND [Serial].[SerialName]=?";
