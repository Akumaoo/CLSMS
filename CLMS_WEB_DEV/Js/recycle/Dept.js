...
$table_MS="";
	...
	$table_MS=$('#table_dept').DataTable({			
	"processing":true,
	"serverSide":true,
	"ordering":true,
	"searching":true,
	"ajax":"SSP/recycle/serverside_dept.php"
		});
	}
...
$('#table_dept').on('draw.dt', function() {
		...
		else
		{
			...
			if(!$checkele)
			{
				$('thead>tr').append('<th id="th_add" class="radio-label-center" tabindex="0" aria-controls="table_dept" rowspan="1" colspan="1" style="width: 63px; font-size:10px">Check All <br><input type="checkbox" id="SA_ser" value="SA"> </th>');
			...
			$('thead>tr>th:nth-child(4)').addClass('collapse');
			$('tbody>tr>td:nth-child(4)').addClass('collapse');
		}
	});

 	$('#cog_action').click(function(){

 		if($('thead>tr>th:nth-child(4)').hasClass('collapse'))
 		{
 			$('thead>tr>th:nth-child(4)').removeClass('collapse');
			$('tbody>tr>td:nth-child(4)').removeClass('collapse');
