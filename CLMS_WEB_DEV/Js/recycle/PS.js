...
$table_MS="";
	...
	$table_MS=$('#table_pending_ser').DataTable({			
	"processing":true,
	"serverSide":true,
	"ordering":true,
	"searching":true,
	"ajax":"SSP/recycle/serverside_Pending_Serials.php"
		});
	}
...
$('#table_pending_ser').on('draw.dt', function() {
		...
		else
		{
			...
			if(!$checkele)
			{
				$('thead>tr').append('<th id="th_add" class="radio-label-center" tabindex="0" aria-controls="table_pending_ser" rowspan="1" colspan="1" style="width: 63px; font-size:10px">Check All <br><input type="checkbox" id="SA_ser" value="SA"> </th>');
