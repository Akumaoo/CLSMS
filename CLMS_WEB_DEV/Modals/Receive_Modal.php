							 ...
							 <?php 
									...
									$sql="Select ReceivedSerialID,asd.DepartmentID,dsa.ReceiveSerialID_Program,dsa.ProgramID,
										...
										(CASE
											...
											WHERE (asd.SerialName=dsa.SerialName OR (asd.SerialName IS NOT NULL AND dsa.SerialName IS NULL)) AND (asd.DateReceiveNotif_Give=dsa.DateReceiveNotif_Give_Prog OR (asd.DateReceiveNotif_Give IS NOT NULL AND dsa.DateReceiveNotif_Give_Prog IS NULL)) AND (asd.Remove IS NULL AND dsa.Remove IS NULL) AND (asd.Status='NotReceived' AND dsa.Status_Prog='NotReceived' OR (asd.Status IS NOT NULL AND dsa.Status_Prog IS NULL)) AND asd.DepartmentID=?
";
									...
									if(sqlsrv_has_rows($query))
									{
										while($row=sqlsrv_fetch_array($query,SQLSRV_FETCH_ASSOC))
										{
											...
											if($type=='Multiple')
											{
												...
												<td>
												<input type="number" name="cont_no" required value="'.$cn.'">
												</td>
												<td>'.$rsprog.'</td>
												<td>'.$SerialName.'</td>
												<td>'.$vn.'</td>
												...
												<td>'.$doi.'</td>
												<td>
												<textarea rows="1" cols="35" name="sc" form="SEND_RECEIVE" required class="text_area"></textarea>
												</td>
												</tr>
													';
											}
